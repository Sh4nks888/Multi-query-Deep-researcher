            ...
            <ul>
              ...
              >
                ...
                >
                  ...
                  <span v-if="task.fallbackUsed" class="fallback-badge">
                    智能降级
                  </span>
                  <span
                    v-if="task.confidenceLabel"
                    class="confidence-badge"
                    :class="confidenceClass(task.confidenceLabel)"
                  >
                    {{ task.confidenceLabel }} {{ formatConfidence(task.confidenceScore) }}
                  </span>
          ...
          <article class="task-detail" v-if="currentTask">
            <header class="task-header">
              ...
              <div class="task-chip-group">
                ...
                <span
                  v-if="currentTaskQueryVariants.length"
                  class="task-label retrieval-chip"
                >
                  多检索：{{ currentTaskQueryVariants.length }} 个查询
                </span>
                <span
                  v-if="currentTaskSearchResultCount > 0"
                  class="task-label retrieval-chip"
                >
                  来源：{{ currentTaskSearchResultCount }} 个
                </span>
                <span
                  v-if="currentTaskConfidenceLabel"
                  class="task-label confidence-chip"
                  :class="confidenceClass(currentTaskConfidenceLabel)"
                  :title="currentTaskConfidenceReason || undefined"
                >
                  置信度：{{ currentTaskConfidenceLabel }}
                  {{ formatConfidence(currentTaskConfidenceScore) }}
                </span>
                <span
            ...
            >
              ...
              <div
                v-if="currentTaskQueryVariants.length"
                class="query-variants"
              >
                <h4>检索查询</h4>
                <ol>
                  <li
                    v-for="(query, index) in currentTaskQueryVariants"
                    :key="`${query}-${index}`"
                  >
                    {{ query }}
                  </li>
              ...
              <p
                v-if="currentTaskConfidenceReason"
                class="confidence-reason"
              >
                {{ currentTaskConfidenceReason }}
              </p>
            </section>
...
const currentTaskQueryVariants = computed(
  () => currentTask.value?.queryVariants ?? []
);
const currentTaskSearchResultCount = computed(
  () => currentTask.value?.searchResultCount ?? 0
);
const currentTaskConfidenceScore = computed(
  () => currentTask.value?.confidenceScore ?? null
);
const currentTaskConfidenceLabel = computed(
  () => currentTask.value?.confidenceLabel ?? null
);
const currentTaskConfidenceReason = computed(
  () => currentTask.value?.confidenceReason ?? null
);
...
function applyFallbackMetadata(
  ...
  const fallbackReason = extractOptionalString(payload.fallback_reason);
  if (fallbackReason) {
    task.fallbackReason = fallbackReason;
  }

  const fallbackStage = extractOptionalString(payload.fallback_stage);
  if (fallbackStage) {
    task.fallbackStage = fallbackStage;
  }
}

function applyRetrievalConfidenceMetadata(
  task: TodoTaskView,
  payload: Record<string, unknown>
): void {
  if (Array.isArray(payload.query_variants)) {
    task.queryVariants = payload.query_variants
      .map((item) => (typeof item === "string" ? item.trim() : ""))
      .filter((item) => item);
  }

  const rawResultCount = payload.search_result_count;
  if (typeof rawResultCount === "number" && Number.isFinite(rawResultCount)) {
    task.searchResultCount = rawResultCount;
  } else if (typeof rawResultCount === "string" && rawResultCount.trim()) {
    const parsed = Number(rawResultCount);
    if (Number.isFinite(parsed)) {
      task.searchResultCount = parsed;
    }
  }

  const rawScore = payload.confidence_score;
  if (typeof rawScore === "number" && Number.isFinite(rawScore)) {
    task.confidenceScore = rawScore;
  } else if (typeof rawScore === "string" && rawScore.trim()) {
    const parsed = Number(rawScore);
    if (Number.isFinite(parsed)) {
      task.confidenceScore = parsed;
    }
  }

  const confidenceLabel = extractOptionalString(payload.confidence_label);
  if (confidenceLabel) {
    task.confidenceLabel = confidenceLabel;
  }

  const confidenceReason = extractOptionalString(payload.confidence_reason);
  if (confidenceReason) {
    task.confidenceReason = confidenceReason;
  }
}
      ...
      (event: ResearchStreamEvent) => {
        if (event.type === "status") {
          ...
          if (task && message) {
            ...
            applyFallbackMetadata(task, payload);
            applyRetrievalConfidenceMetadata(task, payload);
          }
          return;
        }
        ...
        if (event.type === "todo_list") {
          ...
          todoTasks.value = tasks.map((item, index) => {
            ...
            return {
              ...
              fallbackReason:
                typeof item.fallback_reason === "string" && item.fallback_reason.trim()
                  ? item.fallback_reason.trim()
                  : null,
              fallbackStage:
                typeof item.fallback_stage === "string" && item.fallback_stage.trim()
                  ? item.fallback_stage.trim()
                  : null,
              queryVariants,
              searchResultCount,
              confidenceScore,
              confidenceLabel:
                typeof item.confidence_label === "string" && item.confidence_label.trim()
                  ? item.confidence_label.trim()
                  : null,
              confidenceReason:
                typeof item.confidence_reason === "string" && item.confidence_reason.trim()
                  ? item.confidence_reason.trim()
                  : null
            } as TodoTaskView;
          });
        ...
        if (event.type === "task_status") {
          ...
          if (status === "in_progress") {
            ...
            task.confidenceLabel = null;
            task.confidenceReason = null;
...
.task-label.retrieval-chip {
  background: rgba(14, 165, 233, 0.14);
  border-color: rgba(14, 165, 233, 0.32);
  color: #075985;
}

.task-label.confidence-chip {
  font-weight: 600;
}
