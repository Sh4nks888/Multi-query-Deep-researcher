...
class ReportingService:
    ...
    def generate_report(self, state: SummaryState) -> str:
        ...
        tasks_block = []
        for task in state.todo_items: #遍历所有任务
            summary_block = task.summary or "暂无可用信息"
            sources_block = task.sources_summary or "暂无来源"
            # BEGIN retrieval-confidence: expose retrieval coverage to report writer
            query_variants = "\n".join(
                f"  {index}. {query}"
                for index, query in enumerate(task.query_variants or [task.query], start=1)
            )
            confidence_line = (
                f"{task.confidence_label or '未知'}"
            ...
            tasks_block.append(
                ...
                f"- 检索查询：{task.query}\n"
                f"- 多检索查询：\n{query_variants}\n"
                f"- 去重来源数：{task.search_result_count}\n"
                f"- 置信度评分：{confidence_line}\n"
                f"- 评分依据：{task.confidence_reason or '暂无'}\n"
                f"- 执行状态：{task.status}\n"
                f"- 任务总结：\n{summary_block}\n"
                f"- 来源概览：\n{sources_block}\n"
            )
