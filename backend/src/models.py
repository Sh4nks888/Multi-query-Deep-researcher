...
class TodoItem:
    ...
    fallback_stage: Optional[str] = field(default=None)
    # BEGIN retrieval-confidence: task-level retrieval and scoring metadata
    query_variants: list[str] = field(default_factory=list)
    search_result_count: int = field(default=0)
    confidence_score: Optional[float] = field(default=None)
    confidence_label: Optional[str] = field(default=None)
