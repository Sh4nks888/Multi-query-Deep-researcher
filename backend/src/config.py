...
class Configuration(BaseModel):
    ...
    max_concurrent_tasks: int = Field(
        ...
        description="Maximum number of task workers running model calls at once",
    )
    # BEGIN retrieval-confidence: multi-query retrieval settings
    enable_multi_query_retrieval: bool = Field(
        default=True,
        title="Enable Multi-query Retrieval",
        description="Whether each research task should use multiple search queries",
    )
    multi_query_count: int = Field(
        default=5,
        title="Multi-query Count",
        description="Maximum number of query variants generated per research task",
    )
    max_merged_search_results: int = Field(
        default=8,
        title="Max Merged Search Results",
        description="Maximum number of deduplicated sources kept after multi-query search",
    )
    ...
    def from_env(cls, overrides: Optional[dict[str, Any]] = None) -> "Configuration":
        ...
        env_aliases = {
            ...
            "enable_multi_query_retrieval": os.getenv("ENABLE_MULTI_QUERY_RETRIEVAL"),
            "multi_query_count": os.getenv("MULTI_QUERY_COUNT"),
            "max_merged_search_results": os.getenv("MAX_MERGED_SEARCH_RESULTS"),
